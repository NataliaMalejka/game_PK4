...
Boss::Boss(const float& x, const float& y)
{
	...
	attakPair = { {0,true}, {1,true}, {2,true} };
...
void Boss::updateAttack(const sf::Vector2f& playerPosition, std::vector<BossBullet*>& bossBullets, std::vector<Enemy*>& enemies)
{
...
void Boss::attack_3(std::vector<Enemy*>& enemies)
{
	enemies.push_back(new Ghost(sprite.getPosition().x, sprite.getPosition().y));
}

void Boss::update(const sf::Vector2f& playerPosition, std::vector<BossBullet*>& bossBullets, std::vector<Enemy*>& enemies)
{
	...
	if (isStartPosition == false)
	{
		...
		{
			updateAttack(playerPosition, bossBullets, enemies);
		}
	}
	else
	{
