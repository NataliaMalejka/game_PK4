...
GameEngine::GameEngine()
{
	...
	window = new sf::RenderWindow(desktop, "GAME", sf::Style::Fullscreen);
	//window = new sf::RenderWindow(sf::VideoMode(1000,800), "GAME");
	window->setFramerateLimit(60);
	window->setVerticalSyncEnabled(false);

	if (!backgroundTexture.loadFromFile("images/landscape.png"))
	{
		std::cout << "ERROR::GAMEENGINE::GAMEENGINE" << std::endl;
	}

	backgroundSprte.setTexture(backgroundTexture);

	if (!musicBackground.openFromFile("sounds/MusicBackground.ogg"))
	{
		std::cout << "ERROR::GAMEENGINE::GAMEENGINE::MUSIC" << std::endl;
	}

	musicBackground.setLoop(true);
	musicBackground.play();

	if (!soundsItem.openFromFile("sounds/item.ogg"))
	{
		std::cout << "ERROR::GAMEENGINE::GAMEENGINE::SOUNDSITEM" << std::endl;
	}

	if (!soundsHealth.openFromFile("sounds/health.ogg"))
	{
		std::cout << "ERROR::GAMEENGINE::GAMEENGINE::SOUNDSITEM" << std::endl;
	}

	if (!soundsBullet.openFromFile("sounds/laser.ogg"))
	{
		std::cout << "ERROR::GAMEENGINE::GAMEENGINE::SOUNDSITEM" << std::endl;
	}

	backgroundSprte.setScale(static_cast<float>(desktop.width) / backgroundSprte.getLocalBounds().width,
		static_cast<float>(desktop.height) / backgroundSprte.getLocalBounds().height);
...
void GameEngine::start()
{
	...
	backgroundSprte.setScale(320.f / backgroundTexture.getSize().x, 240.f / backgroundTexture.getSize().y);
...
void GameEngine::loadMap()
{
	...
	backgroundSprte.setPosition(player->getPosition().x - backgroundSprte.getGlobalBounds().width / 2.f,
		player->getPosition().y - backgroundSprte.getGlobalBounds().height / 2.f);
...
void GameEngine::update()
{
	...
	backgroundSprte.setPosition(player->getPosition().x - 160.f, player->getPosition().y - 120.f);
	...
	{
		enemy->update(player->getPosition(), bossBullets);	
	}
...
void GameEngine::render()
{
	...
	window->draw(backgroundSprte);

	maps[mapNumer]->render(window);

	if (imageMapNumer[mapNumer] == "images/map3.png"  && enemies.size()==0)
	{
		winText.setScale(0.25f, 0.25f);
		winText.setPosition((view->getCenter().x - winText.getGlobalBounds().width / 2.f), view->getCenter().y - 30.f);
		window->draw(winText);
	}
